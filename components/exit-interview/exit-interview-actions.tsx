...
import {
  ...
  cancelExitInterviewAction,
} from "@/app/(workspace)/hr/exit-interviews/actions";

export function ExitInterviewActionsBar({
  id,
  docstatus,
}: {
  id: string;
  ...
  if (docstatus === 2) {
    return <p className="text-xs text-ash-500">This interview is cancelled.</p>;
  }

  const onSubmit = () =>
    start(async () => {
      const r = await submitExitInterviewAction(id);
  ...
  const onCancel = () =>
    start(async () => {
      ...
      const r = await cancelExitInterviewAction(id);
      if (!r.ok) window.alert(r.error);
      else router.refresh();
    });
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {docstatus === 1 && (
        ...
        >
          <X className="h-4 w-4" />
          {pending ? "Working…" : "Cancel interview"}
        </button>
      )}
    </div>
  );
