...
/**
 * Interactive core of /recruitment/candidates. The parent server
 * component fetches the initial candidates + jobs lists via
 * fetchCandidatesInitial() so first paint has real data — no
 * spinner, no useEffect round-trip on mount. Filter-change
 * refetches still happen client-side through apiClient (with the
 * SWR read cache). See page.tsx for the SSR shell.
 */
export function CandidatesClient({
    ...
    // No spinner on first mount — we start with server-rendered data.
    // Filter switches flip this back true via loadCandidates below.
    const [loading, setLoading] = useState(false)
    const [searchTerm, setSearchTerm] = useState("")
    const [jobs, setJobs] = useState<JobPosting[]>(initial.jobs)
    ...
    // Load jobs only if the server-shell handed us an empty list
    // (rare — a fresh tenant, or the server call failed). Otherwise
    // rely on the initial prop.
    useEffect(() => {
    ...
    // Refetch on filter change. We DON'T flip `loading` back to true
    // here — the previous rows stay visible while the new data
    // arrives (SWR-style), and the apiClient SWR cache usually
    // returns instantly for repeat filter combos. First mount is a
    // no-op-ish repeat of the server fetch (deduped by apiClient's
    // in-flight map) so the UI never blanks out on load.
    useEffect(() => {
