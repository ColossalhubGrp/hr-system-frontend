...
import { submitFnfAction, cancelFnfAction } from "@/app/(workspace)/hr/full-and-final/actions";

export function FnfActionsBar({
  id,
  docstatus,
}: {
  id: string;
  ...
  if (docstatus === 2) {
    return <p className="text-xs text-ash-500">This statement is cancelled.</p>;
  }

  const onSubmit = () =>
    start(async () => {
      const r = await submitFnfAction(id);
      if (!r.ok) window.alert(r.error);
      else router.refresh();
    });
  const onCancel = () =>
    start(async () => {
      if (!window.confirm("Cancel this FnF statement? Will reverse any GL entries.")) return;
      const r = await cancelFnfAction(id);
      if (!r.ok) window.alert(r.error);
      else router.refresh();
    });
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {docstatus === 1 && (
        ...
        >
          <X className="h-4 w-4" />
          {pending ? "Working…" : "Cancel statement"}
        </button>
      )}
    </div>
  );
