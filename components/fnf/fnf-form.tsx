...
import type { FormState } from "@/app/(workspace)/hr/full-and-final/actions";

const EMPTY: FormState = {};
type Action = (prev: FormState, form: FormData) => Promise<FormState>;

export function FnfForm({
  action,
  employeeDirectory,
  companies,
  cancelHref = "/hr/full-and-final",
}: {
  action: Action;
  ...
  return (
    <form action={dispatch} className="flex flex-col gap-5">
      ...
      <FormSection
        title="Statement"
        description="Save a blank statement, then pull payables/receivables from the detail page."
      >
        <EmployeePickerField
          name="employee"
          required
        ...
        />
        <Field label="Relieving date" htmlFor="relieving_date" required error={fe.relieving_date}>
          <TextInput
            id="relieving_date"
            name="relieving_date"
            type="date"
            invalid={Boolean(fe.relieving_date)}
          />
        </Field>
...
function Submit() {
  ...
  return (
    ...
    >
      ...
      {pending ? "Saving…" : "Create statement"}
    </button>
  );
}
