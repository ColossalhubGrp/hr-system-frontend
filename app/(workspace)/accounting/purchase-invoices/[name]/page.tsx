...
import { getPurchaseInvoice } from "@/lib/frappe/purchase-invoice";
import { PurchaseInvoiceActions } from "@/components/accounting/purchase-invoice-actions";

export const dynamic = "force-dynamic";

export async function generateMetadata({ params }: { params: { name: string } }) {
  return { title: `${decodeURIComponent(params.name)} · Purchase Invoice · Colossal HR` };
}

export default async function PurchaseInvoiceDetailPage({ params }: { params: { name: string } }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <PageHeader
        icon={ReceiptText}
        crumb={`Accounting · Purchase Invoices · ${doc.name}`}
        title={doc.name}
        subtitle={`${doc.supplierName ?? doc.supplier} · ${doc.postingDate} · ${doc.company}`}
        actions={
          <div className="flex items-center gap-3">
            <StatusPill status={doc.status || (doc.docstatus === 0 ? "Draft" : doc.docstatus === 1 ? "Submitted" : "Cancelled")} />
            <PurchaseInvoiceActions name={doc.name} docstatus={doc.docstatus} />
          </div>
        }
      />
      ...
      <section className="rounded-2xl border border-border/60 bg-card p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Detail label="Supplier" value={doc.supplierName ?? doc.supplier} />
          ...
          <Detail label="Supplier bill no." value={doc.billNo ?? "—"} />
          <Detail label="Bill date" value={doc.billDate ?? "—"} />
        </div>
      </section>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border/60 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                ...
                <th className="py-2 text-left">Expense Account</th>
            ...
            <tbody>
              {doc.items.map((it) => (
                <tr key={it.idx} className="border-b border-border/30 last:border-0">
                  ...
                  <td className="py-2 pr-2 text-muted-foreground">{it.expenseAccount ?? "—"}</td>
