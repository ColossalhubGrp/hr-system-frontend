...
/**
 * Interactive core for /recruitment/interviewreports — the aggregated
 * dashboard. Parent server component fetches all interview sessions
 * via fetchInterviewsInitial() so the KPI tiles, breakdown bars, and
 * recent list all paint from real data with no spinner. A silent
 * background refresh keeps the numbers current on mount.
 */
export function InterviewReportsClient({
  initial,
}: {
  initial: InterviewSession[]
}) {
  const [sessions, setSessions] = useState<InterviewSession[]>(initial)
