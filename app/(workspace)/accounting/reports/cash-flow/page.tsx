...
import { ArrowLeftRight, ChevronLeft, Search } from "lucide-react";
import { PageHeader } from "@/components/common/page-header";
import { listCompanies } from "@/lib/frappe/accounting";
import { cashFlow } from "@/lib/frappe/accounting-reports";
import { ReportTable } from "@/components/accounting/report-table";
import { FrappeRequestError } from "@/lib/frappe/client";

export const metadata = { title: "Cash Flow · Accounting · Colossal HR" };
...
export default async function CashFlowPage({ searchParams }: { searchParams: SP }) {
  ...
  let report: Awaited<ReturnType<typeof cashFlow>> | null = null;
  let error: string | null = null;
  if (company) {
    try {
      report = await cashFlow({ company, fromDate: from, toDate: to, periodicity });
    } catch (e) {
      error =
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <PageHeader
        icon={ArrowLeftRight}
        crumb="Accounting · Reports · Cash Flow"
        title="Cash Flow"
        subtitle="Operating, investing and financing cash movement over the period."
      />

      <form action="/accounting/reports/cash-flow" className="flex flex-wrap items-end gap-3 rounded-2xl border border-border/60 bg-card p-3">
      ...
      {error ? (
        ...
        <ReportTable columns={report.columns} rows={report.result} empty="No cash flow rows for this period." />
      )}
    </div>
  );
}
