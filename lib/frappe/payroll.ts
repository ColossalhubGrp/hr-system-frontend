...
export async function listSalaryStructures(opts: {
  active?: string;
  company?: string;
  page?: number;
  pageSize?: number;
  ...
  if (opts.active === "Yes") filters.push(["is_active", "=", "Yes"]);
  if (opts.active === "No") filters.push(["is_active", "=", "No"]);
  ...
  const [rowsRaw, totalRaw, active, inactive] = await Promise.all([
    frappeCall<Row[]>({
      method: "frappe.client.get_list",
      args: {
    ...
    frappeCall<number>({
      method: "frappe.client.get_count",
      args: { doctype: "Salary Structure", filters: JSON.stringify(filters) },
      as: "user",
    }).catch(() => 0),
    frappeCall<number>({
      method: "frappe.client.get_count",
      args: {
        doctype: "Salary Structure",
        filters: JSON.stringify([["is_active", "=", "Yes"]]),
      },
      as: "user",
    }).catch(() => 0),
    frappeCall<number>({
      ...
      args: {
        doctype: "Salary Structure",
        filters: JSON.stringify([["is_active", "=", "No"]]),
      },
      as: "user",
    }).catch(() => 0),
  ]);
  ...
  return {
    rows: rowsRaw.map((r) => ({
      ...
      name: r.name,
      company: r.company,
      isActive: (r.is_active ?? "").toLowerCase() === "yes",
    ...
    page,
    pageSize,
    counts: { active: Number(active ?? 0), inactive: Number(inactive ?? 0) },
  };
}
