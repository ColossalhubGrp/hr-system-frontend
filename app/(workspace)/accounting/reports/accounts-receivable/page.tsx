...
import { FileOutput, ChevronLeft, Search } from "lucide-react";
import { PageHeader } from "@/components/common/page-header";
import { listCompanies } from "@/lib/frappe/accounting";
import { accountsReceivable } from "@/lib/frappe/accounting-reports";
import { ReportTable } from "@/components/accounting/report-table";
import { FrappeRequestError } from "@/lib/frappe/client";

export const metadata = { title: "Accounts Receivable · Accounting · Colossal HR" };
export const dynamic = "force-dynamic";

type SP = { company?: string; date?: string };

export default async function AccountsReceivablePage({ searchParams }: { searchParams: SP }) {
  const companies = await listCompanies();
  const company = searchParams.company || companies[0]?.name || "";
  const today = new Date().toISOString().slice(0, 10);
  const date = searchParams.date || today;

  let report: Awaited<ReturnType<typeof accountsReceivable>> | null = null;
  let error: string | null = null;
  if (company) {
    try {
      report = await accountsReceivable({ company, reportDate: date });
    } catch (e) {
      error =
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <PageHeader
        icon={FileOutput}
        crumb="Accounting · Reports · Accounts Receivable"
        title="Accounts Receivable"
        subtitle="Money owed to us, aged into 30/60/90/120-day buckets."
      />

      <form action="/accounting/reports/accounts-receivable" className="flex flex-wrap items-end gap-3 rounded-2xl border border-border/60 bg-card p-3">
      ...
      {error ? (
        ...
        <ReportTable columns={report.columns} rows={report.result} empty="Nothing outstanding as of this date." />
      )}
    </div>
  );
}
