...
import {
  createSalesTaxTemplateAction,
  updateSalesTaxTemplateAction,
  deleteSalesTaxTemplateAction,
  type FormState,
} from "@/app/(workspace)/accounting/tax/sales-templates/actions";
import type { AccountOption } from "@/lib/frappe/accounting";
import { CHARGE_TYPES } from "@/lib/frappe/tax/sales-template-constants";
...
const EMPTY_LINE = (): Line => ({ charge_type: "On Net Total", account_head: "", description: "", rate: "0", cost_center: "", included_in_print_rate: false });
...
export function SalesTaxTemplateForm({
  mode,
  name,
  companies,
  accounts,
  ...
  const action = mode === "create" ? createSalesTaxTemplateAction : updateSalesTaxTemplateAction.bind(null, name ?? "");
  ...
  return (
    <form action={dispatch} className="flex flex-col gap-5">
      ...
      <FormSection title="Template" description="Reusable tax block. Set as default for one company and every new Sales Invoice pre-selects it.">
        <div className="grid gap-4 sm:grid-cols-3">
          <Field label="Title" htmlFor="title" error={fe.title} required>
            <TextInput id="title" name="title" defaultValue={initial?.title ?? ""} placeholder="e.g. Zimbabwe VAT 15%" />
          </Field>
      ...
      <FormSection title="Charges" description="Each row is a line on the invoice tax table.">
          ...
          {lines.map((l, idx) => (
            <div key={idx} className="grid grid-cols-12 items-end gap-2 rounded-xl border border-border/60 bg-muted/10 p-3">
              ...
              <div className="col-span-8 md:col-span-3">
                ...
                <TextInput value={l.description} onChange={(e) => setLines((p) => p.map((x, i) => (i === idx ? { ...x, description: e.target.value } : x)))} placeholder="Shown on invoice" />
              </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Link href={"/accounting/tax/sales-templates" as Route} className="rounded-chip border border-input px-4 py-2 text-sm font-semibold hover:bg-muted/40">Cancel</Link>
          <SubBtn mode={mode} />
        </div>
      </div>
    </form>
...
function DelBtn({ name }: { name: string }) {
  const onSubmit = async () => { await deleteSalesTaxTemplateAction(name); };
  return (
