...
import { submitTravelAction, cancelTravelAction } from "@/app/(workspace)/hr/travel/actions";

export function TravelActionsBar({
  id,
  docstatus,
}: {
  id: string;
  ...
  if (docstatus === 2) {
    return <p className="text-xs text-ash-500">This request is cancelled.</p>;
  }

  const onSubmit = () =>
    start(async () => {
      const r = await submitTravelAction(id);
  ...
  const onCancel = () =>
    start(async () => {
      ...
      const r = await cancelTravelAction(id);
      if (!r.ok) window.alert(r.error);
      else router.refresh();
    });
