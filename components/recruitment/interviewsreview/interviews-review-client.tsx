...
/**
 * Interactive core of /recruitment/interviewsreview. Parent server
 * component fetches the full session list via
 * fetchInterviewsInitial() so first paint carries real rows — no
 * spinner. A background refresh still fires so freshly-recorded
 * completions and scores update in place. See page.tsx for the
 * server shell.
 */
export function InterviewsReviewClient({
  initial,
}: {
  initial: InterviewSession[]
}) {
  const [interviews, setInterviews] = useState<InterviewSession[]>(initial)
  // No spinner on first mount — data is already in state.
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [search, setSearch] = useState('')
  const [filter, setFilter] = useState<FilterKey>('all')

  // Background refresh — SWR cache in apiClient dedupes if the
  // server-side call is still fresh; otherwise a quiet backend hit
  // brings in newer rows. The user never sees a spinner because we
  // never flip `loading` back to true here.
  useEffect(() => {
    let cancelled = false
    const run = async () => {
      try {
