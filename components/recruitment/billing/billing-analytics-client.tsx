...
/**
 * Shared interactive core for the three billing-analytics pages.
 * Three server shells hand in the initial analytics payload + the
 * back-to-billing href + whether the "All time" filter is offered
 * (the hr/ variant hides it by convention; root + employer show it).
 * Deduplicates ~1000 lines that were previously copy-pasted.
 */
export function BillingAnalyticsClient({
    initial,
    backHref,
    showAllTime,
}: {
    initial: BillingAnalytics | null
    backHref: string
    showAllTime: boolean
}) {
    const [range, setRange] = useState<BillingRange>("period")
    const [analytics, setAnalytics] = useState<BillingAnalytics | null>(initial)
    // No spinner on first mount — initial is already in state.
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState<string | null>(null)

    // The server-side initial fetch was for range="period" (the
    // component's default). When the user changes the range, we
    // refetch on the client. Skip the first-mount fetch since
    // it's identical to what the server just did (apiClient's
    // SWR cache would dedupe it anyway, but skipping is cleaner).
    const isFirstRun = useMemo(() => ({ current: true }), [])
    useEffect(() => {
        if (isFirstRun.current && range === "period") {
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {([
                            ...
                            // "All time" is offered on root + employer
                            // billing but was intentionally hidden on
                            // the hr/ variant. Gate on the prop so
