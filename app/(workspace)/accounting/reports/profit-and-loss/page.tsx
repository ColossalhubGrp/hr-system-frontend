...
import { profitAndLoss } from "@/lib/frappe/accounting-reports";
import { ReportTable } from "@/components/accounting/report-table";
import { FrappeRequestError } from "@/lib/frappe/client";

export const metadata = { title: "Profit and Loss · Accounting · Colossal HR" };
export const dynamic = "force-dynamic";

type SP = { company?: string; from?: string; to?: string; periodicity?: string };

const PERIODICITIES = ["Monthly", "Quarterly", "Half-Yearly", "Yearly"];

export default async function ProfitAndLossPage({ searchParams }: { searchParams: SP }) {
  ...
  const periodicity = searchParams.periodicity || "Monthly";

  let report: Awaited<ReturnType<typeof profitAndLoss>> | null = null;
  let error: string | null = null;
  if (company) {
    try {
      report = await profitAndLoss({ company, fromDate: from, toDate: to, periodicity });
    } catch (e) {
      error =
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <PageHeader
        icon={LineChart}
        crumb="Accounting · Reports · Profit and Loss"
        title="Profit and Loss"
        subtitle="Income minus expenses, sliced by the period you choose."
      />

      <form action="/accounting/reports/profit-and-loss" className="flex flex-wrap items-end gap-3 rounded-2xl border border-border/60 bg-card p-3">
      ...
      {error ? (
        ...
        <ReportTable columns={report.columns} rows={report.result} empty="No P&L rows for this period." />
      )}
    </div>
  );
}
