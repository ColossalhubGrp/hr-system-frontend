...
/**
 * Interactive core of /recruitment/candidates/profile/[email]. The
 * parent server component fetches the profile via
 * fetchCandidateProfile(decodedEmail) so first paint has real data
 * — no spinner. If the server call returned an error payload, we
 * hydrate `error` from it and skip the client refetch.
 */
export function CandidateProfileClient({
    initial,
    decodedEmail,
}: {
    initial: { success?: boolean; error?: string } | null
    decodedEmail: string
}) {
    const router = useRouter()
    const initialSucceeded = Boolean(initial?.success)

    const [profile, setProfile] = useState<any>(
        initialSucceeded ? initial : null,
    )
    ...
    const [error, setError] = useState<string | null>(
        !initial
            ...
            : !initialSucceeded
                ? initial.error || "Failed to load candidate profile"
                : null,
    )

    useEffect(() => {
        // If the server-side fetch already gave us a good profile,
        // skip the initial client refetch. The apiClient SWR cache
        // would dedupe anyway, but skipping keeps this cleaner.
        if (initialSucceeded) return
        loadProfile()
