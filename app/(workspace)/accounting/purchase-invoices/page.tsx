...
export default async function PurchaseInvoicesPage({ searchParams }: { searchParams: SP }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <PageHeader
        ...
        subtitle={`${result.total.toLocaleString()} bills in view.`}
        actions={
          <Link
            href={"/accounting/purchase-invoices/new" as Route}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        ...
        <SummaryTile label="Payable" value={formatMoney(result.counts.outstandingTotal)} hint="Sum owed" icon={Coins} tone="ink" />
      </div>
      ...
      <DataTable<PurchaseInvoiceRow>
        rows={result.rows}
        rowKey={(r) => r.name}
        rowHref={(r) => `/accounting/purchase-invoices/${encodeURIComponent(r.name)}`}
        empty={
          <div className="flex flex-col items-center gap-2 py-10 text-center">
            <ReceiptText className="h-8 w-8 text-muted-foreground" />
            <p className="text-sm text-muted-foreground">No purchase invoices match this view.</p>
          </div>
        }
        columns={[
          { header: "Bill", cell: (r) => <span className="font-mono text-sm">{r.name}</span> },
          { header: "Supplier", cell: (r) => r.supplierName ?? r.supplier },
    ...
    <div className="flex flex-wrap items-center gap-1 rounded-chip border border-border/60 bg-muted/20 p-0.5">
      {STATUS_OPTIONS.map((s) => {
        ...
        return (
          <Link
            key={s}
            href={`/accounting/purchase-invoices?${params.toString()}` as Route}
...
function Pagination({ page, pageSize, total, sp }: { page: number; pageSize: number; total: number; sp: SP }) {
  const build = (n: number) => {
    ...
    p.set("page", String(n));
    return `/accounting/purchase-invoices?${p.toString()}`;
  };
  return (
