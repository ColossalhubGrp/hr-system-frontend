...
/**
 * Shared interactive core for /recruitment/billing,
 * /recruitment/hr/billing and /recruitment/employer/billing. Three
 * server shells (one per URL prefix) fetch the summary via
 * fetchBillingSummaryForCurrentUser() and hand it in as `initial`
 * along with the correct `analyticsHref` for their surface.
 *
 * Deduplicates ~350 lines that were previously copy-pasted across
 * the three routes — future edits touch one file.
 */
export function BillingClient({
    ...
    // Background refresh so numbers stay live. SWR cache in
    // apiClient dedupes against the recent server-side call.
    useEffect(() => {
        let cancelled = false
        ;(async () => {
            try {
